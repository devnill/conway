...
//! allocations shared across siblings (architecture §5.1, §7).
//!
//! Lives in `conway-core`, not `conway-session`. This is pure logic over the
//! `SessionStore` port (every method below is generic over `S: SessionStore +
//! ?Sized`), not over `JsonlSessionStore` specifically, so it belongs beside
//! the contract rather than inside the one adapter that happened to host it
//! first (board item 01KZVYVTVWRH20R6VJ6G3SWTJ6, "Stage 1a" -- cut the
//! runtime's forced dependency on the JSONL adapter). `conway-session`
//! re-exports this type unchanged for existing callers.
//!
//! ## Algorithm
//!
