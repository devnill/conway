...
//!
//! No git clone, no archive extraction (`Cargo.toml`'s own doc has the
//! full argument), no digest check, no allow-list, no trust prompt
//! (`docs/plugins/trust-and-security.md`'s ruling, restated at the top of
//! this doc). Everything a fetched plugin's directory declares runs with
//! the operator's own privileges once conway loads it, on the identical
...
pub use manifest::{
    fetch_marketplace, MarketplaceManifest, MarketplacePluginEntry, MAX_MANIFEST_BYTES,
};
