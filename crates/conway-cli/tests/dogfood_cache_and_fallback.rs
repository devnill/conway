...
/// proves instead.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
#[ignore = "blocked on board item 01M2NT91W9R9H0943HA8CFPTKM: diagnosed as a real product defect, not a test/capture artifact -- `switch_session` (`tui/commands.rs`) pushes the `switched model to ...` Notice onto `state.transcript`, then returns `Effect::FocusNewSession`. The SAME keypress's synchronous handling (`tui/app/run.rs`'s `FocusNewSession` arm -> `App::try_focus_agent` -> `AppState::focus_agent`, `tui/state.rs`) unconditionally runs `self.transcript.clear()` on the very next line, wiping the notice just pushed, before the run loop's one `if dirty { terminal.draw(..) }` check (`tui/app/run.rs` line ~140) ever fires -- so the notice is created and destroyed within a single event-processing tick and is never rendered, on every `/model` switch. This directly contradicts `switch_session`'s own doc comment (\"Pushes a Notice ... so the transcript records the switch itself\"). Fix belongs in product code (`AppState::focus_agent`/`try_focus_agent`), out of this test's scope."]
