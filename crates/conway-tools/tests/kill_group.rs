...
//! executable file), and every one of them is launched via
//! `conway_test_fixtures::script_command("/bin/sh", path)`, not
//! `Command::new(path)`'s direct `execve` of the file `fixture()` just wrote
//! and closed moments earlier -- the fix for board item
//! `01M3CCB39A1SKWFQEKC2CZK7S9` (CI runs `36052363927` and `36100724242`,
//! both panicking at the spawn step of
...
//! since generalised into `conway-test-fixtures` (board item
//! `01M3CMQZZCSRF7YB9GEMA92M0C`) as the ONE implementation (P-14) three more
//! call sites, across three other test files, also now use -- see that
//! crate's own doc for the full mechanism, the measurements behind it, and
//! (its own doc, "Two call sites that deliberately do NOT use this") the two
//! sibling helpers that were measured and deliberately left unconverted.
...
async fn warm_backgrounding_fixture(path: &Path, marker: &Path) {
    let mut command = script_command("/bin/sh", path);
    command
