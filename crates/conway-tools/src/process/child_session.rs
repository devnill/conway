...
//!
//! **Each crate's own public error enum is UNCHANGED -- observably.**
//! `McpPluginError` and `SubprocessPluginError` keep their own variants and
//! their own `thiserror` `Display` text -- callers see no difference.
//! `Spawn`/`TimedOut` are no longer HAND-duplicated, though: both enums'
//! own definitions of those two variants are generated by
//! `child_session_error_taxonomy!` (item `01M250FJFWHD5FV5JFBYZN1W2C`),
//! which this module also owns, so the field names/types/doc comments live
//! once and each crate supplies only its own already-differing `Display`
//! text. Each crate maps this module's four shared causes onto its own
//! error type by implementing [`ChildSessionError`], a one-line-per-variant
//! translation
...
/// Generates one `ChildSessionError` implementor's whole public error enum:
/// the `Spawn`/`TimedOut` variants every consumer needs (item
/// `01M250FJFWHD5FV5JFBYZN1W2C`, the OTHER half of this module's own
/// consolidation -- board item `01M0TV7ZDS8X4F4TEJPRZB9P6T` unified the
/// MECHANISM above and said, in its own commit message, that
/// `McpPluginError` and `SubprocessPluginError` stayed duplicated by hand:
/// same variants, same fields, same doc comments, differing only in the
/// "MCP " prefix each crate's own `Display` text carries), plus, verbatim,
/// whatever crate-specific variants (`NonzeroExit`, `SessionDied`, and the
/// rest) the caller appends after them.
///
/// **Why a macro, not a wrapped/embedded type.** A `Spawn(SharedStruct)`
/// tuple variant was the first design tried here and does not survive
/// contact with this item's own "existing test suites pass unedited"
/// constraint: both crates' test suites already pattern-match `Spawn`/
/// `TimedOut` by NAMED FIELD directly on the outer variant --
/// `conway-plugin-mcp/tests/mcp_end_to_end.rs`'s `McpPluginError::Spawn {
/// ref config_id, .. }` and `conway-plugin-subprocess/tests/mechanism.rs`'s
/// `SubprocessPluginError::TimedOut { after_ms: 500, .. }` -- and a tuple
/// wrapper changes exactly that shape. Expanding THIS macro instead
/// produces an ordinary, unwrapped enum with the SAME named-field variants
/// either crate always had, so every existing pattern keeps compiling
/// unedited; only the SOURCE TEXT of the field declarations and their doc
/// comments now lives once, here.
///
/// `$spawn_display`/`$timeout_display` stay two SEPARATE literals, not one
/// shared literal composed with `concat!` -- thiserror's `#[error("...")]`
/// shorthand parses its argument as a literal string at macro-expansion
/// time to find `{field}` placeholders, and does not expand a nested
/// `concat!` invocation written inside that position first, so the "MCP "
/// prefix cannot be spliced in that way. Each crate supplies its own
/// COMPLETE, already-differing text instead (unchanged from what it wrote
/// by hand before this item).
#[macro_export]
macro_rules! child_session_error_taxonomy {
    (
        $(#[$meta:meta])*
        $vis:vis enum $name:ident {
            spawn: $spawn_display:literal,
            timeout: $timeout_display:literal,
            $($rest:tt)*
        }
    ) => {
        $(#[$meta])*
        ...
        $vis enum $name {
            /// The configured command could not even be spawned: not found,
            /// not executable, or any other OS-level spawn failure. Also
            /// covers a malformed spec this host cannot even attempt (an
            /// empty `command`).
            #[error($spawn_display)]
            Spawn { config_id: String, detail: String },
            /// A round trip's per-call deadline elapsed and the process
            /// group was killed (graceful SIGTERM-then-SIGKILL,
            /// `conway_tools::process::unix::kill_group`).
            #[error($timeout_display)]
