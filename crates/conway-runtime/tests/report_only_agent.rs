...
use conway_core::ports::{
    Backend, GenerateResponse, HealthRegistry, PermissionGate, Plugin, PluginConfig, Router,
...
use conway_runtime::context::SystemPromptSpec;
...
async fn report_only_proposer_spawned_via_subagent_host_runs_end_to_end() {
    ...
    let (runtime, _store) = runtime_with_plugins(
        backend.clone(),
        vec![Arc::new(FakePlugin::new(vec![tool]))],
        defs,
    );
