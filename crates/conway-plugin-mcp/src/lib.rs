...
/// live once, shared with `conway-plugin-subprocess::SubprocessPluginError`;
/// only the `Display` text below is this crate's own.
pub enum McpPluginError {
    spawn: "MCP plugin '{config_id}' failed to spawn: {detail}",
    timeout: "MCP plugin '{config_id}' timed out after {after_ms}ms",
...
/// `McpPluginError::Spawn`/`TimedOut`'s exact `Display` text. Written to
/// pass unchanged whether `Spawn`/`TimedOut` are hand-written directly on
...
mod spawn_timeout_display_pin {
    use super::McpPluginError;
    use conway::plugin::ChildSessionError;

    #[test]
    fn spawn_display_matches_pre_consolidation_text() {
        let err = McpPluginError::spawn("my-server", "not found".to_string());
        assert_eq!(
            err.to_string(),
            "MCP plugin 'my-server' failed to spawn: not found"
        );
    }

    #[test]
    fn timed_out_display_matches_pre_consolidation_text() {
        let err = McpPluginError::timed_out("my-server", 5000);
        assert_eq!(
            err.to_string(),
            "MCP plugin 'my-server' timed out after 5000ms"
        );
    }
}
