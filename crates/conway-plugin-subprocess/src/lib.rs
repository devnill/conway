...
conway::plugin::child_session_error_taxonomy! {
...
///
/// **`Spawn`/`TimedOut` are generated, not hand-written -- see
/// `conway::plugin::child_session_error_taxonomy!`'s own doc (item
/// `01M250FJFWHD5FV5JFBYZN1W2C`).** Their field names/types/doc comments
/// live once, shared with `conway-plugin-mcp::McpPluginError`; only the
/// `Display` text below is this crate's own.
pub enum SubprocessPluginError {
    spawn: "plugin '{config_id}' failed to spawn: {detail}",
    timeout: "plugin '{config_id}' timed out after {after_ms}ms",
...
/// Item `01M250FJFWHD5FV5JFBYZN1W2C`'s own load-bearing check -- pins
/// `SubprocessPluginError::Spawn`/`TimedOut`'s exact `Display` text. Written
/// to pass unchanged whether `Spawn`/`TimedOut` are hand-written directly on
/// this enum (as they were before this item) or generated by
/// `conway::plugin::child_session_error_taxonomy!` (as they are after): it
/// only calls the public [`ChildSessionError`] constructors and asserts on
/// `.to_string()`, never on the enum's own definition shape. That is what
/// makes it a CHECK that this item's consolidation left operator-visible
/// text alone, not a description of the current code.
#[cfg(test)]
mod spawn_timeout_display_pin {
    use super::SubprocessPluginError;
    use conway::plugin::ChildSessionError;

    #[test]
    fn spawn_display_matches_pre_consolidation_text() {
        let err = SubprocessPluginError::spawn("my-plugin", "not found".to_string());
        assert_eq!(
            err.to_string(),
            "plugin 'my-plugin' failed to spawn: not found"
        );
    }

    #[test]
    fn timed_out_display_matches_pre_consolidation_text() {
        let err = SubprocessPluginError::timed_out("my-plugin", 5000);
        assert_eq!(err.to_string(), "plugin 'my-plugin' timed out after 5000ms");
    }
}
