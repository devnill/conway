...
**The operator reviewer drives the TUI itself, via `tmux`.** It no longer hands
you a script to run by hand — `tmux` is a real pty and `lens-operator.md` §3 has
the mechanism. Expect its findings to cite `capture-pane` output as evidence. A
return that says "TUI not driven — no pty" is a defective return: send it back.

**Relay only judgement questions.** If the reviewer returns a `## Operator
judgement` section, it is asking what a screen capture cannot answer — whether a
wait was tolerable, whether an error read as helpful. Hand those over verbatim,
wait, and pass the answers back to the same reviewer (continue it; do not start a
fresh one) so it can fold them in and finalise. Do not synthesise the operator
territory until that final return is in hand. If there is no such section,
nothing is owed and you proceed.
...
| 2026-09-16 | **The reviewer drives the TUI itself: `tmux` is a pty, and the premise that it could not was false.** The 2026-09-09 amendment below built a hand-off protocol around "a subagent has no interactive terminal". That is wrong — `tmux` gives a real pty, the TUI cannot distinguish it from a terminal, and it is driven entirely from `Bash` with no MCP server and no human in the loop. Three rounds of "TUI not driven — no pty" were avoidable. `lens-operator.md` §3 now carries the `new-session`/`send-keys`/`capture-pane` recipe and the two failures that cost a retry each (the pane dies when the command exits — wrap in `sh -c '...; sleep 600'`; size the window `-x 200 -y 50` or the TUI wraps at 80 columns). Driving is now expected coverage, not a stretch goal, and "TUI not driven — no pty" is no longer an acceptable line in **Not checked**. The operator is retained for judgement only — whether something *feels* wrong — never for transcription. **Evidence this was the right call:** the first tmux-driven run found that guided setup writes `settings.json` to the config dir and `models.json` to the project dir, so `cd` collapses the context window 12.8x — the root cause of a week-old board item that two reviewers had looked at and recorded only as a symptom. |
| 2026-09-16 | **Six lens amendments from the 2026-09-10 round folded in (`STATE-OF-THE-UNION.md` §9), read against the tmux amendment above so none of them reintroduced pty-impossibility language.** (1) `lens-operator.md` §3 and `lens-caller.md` §2 now say **`CONWAY_CONFIG_DIR` plus a working directory outside `$HOME`**, not "isolated `$HOME`" — `HOME=` alone does not isolate a run, since conway's config search still walks ancestors to find the operator's real `~/.conway`. (2) Both lenses now say to record `steps_taken` alongside `usage.input_tokens` and **discard a weight run where it is not 1** — only a single-step run is comparable across rounds. (3) `lens-sustainability.md` §3.2's helper-duplication table was a fixed snapshot from when the lens was first written (36/52/46 files); re-measured today it is 0/1/3, because `crates/conway/src/test_support.rs` already absorbed most of it. Replaced the frozen numbers with a re-run-each-round recipe and today's count as a dated data point, not a new permanent calibration. (4) Tool-call ranges widened for the three lenses that actually ran the binary and went over: `lens-surfaces.md` 30–40→30–45 (used 46), `lens-caller.md` 25–40→25–45 (used 43), `lens-sustainability.md` 30–45→30–50 (used 46). (5) `lens-operator.md` §3 says how to bound a run on macOS, which ships no `timeout`(1) — inside `tmux`, cap your own `capture-pane` polling loop and kill the session when you hit it; outside it, background the command, capture the PID, `sleep`, `kill`. (6) `lens-operator.md` §3 says a return with an operator judgement question still outstanding is provisional on that answer, separate from what the reviewer already drove and found itself. |
