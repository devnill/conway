...
| [`REVIEW-daily-driver-cli.md`](REVIEW-daily-driver-cli.md) | The CLI judged as a daily driver against Claude Code, Codex, OpenCode, Cursor, Hermes and Antigravity (2026-09-07): scorecard, prioritized findings cited to source and to real session transcripts, ideas worth wanting, what not to build, and the operator decisions it asks for. | Snapshot. Superseded by the next daily-driver review. |
...
| [`DESIGN-surface-coherence.md`](DESIGN-surface-coherence.md) | One home per thing: the three surface kinds (action/view/configuration), the six rules for where a TUI command or menu entry lives, the corrected persistent-versus-session split, and familiarity as convergence across independent terminal coding harnesses rather than deference to one. | Living. Amended in place, append-only. |
