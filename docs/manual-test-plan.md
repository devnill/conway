...
| 5.6 | Skills — try `/ideate:refine` | **Expected to WORK now** (board item `01M1DG5TTF6NHW2RXJRZ8ZPE7K` reversed the earlier "skills and agents are OUT OF SCOPE" ruling): resolves and submits `skills/refine/SKILL.md`'s own body — the `:` you'd type in Claude Code itself is accepted as an alias for conway's own `/ideate.refine`. All five of ideate's real skills should resolve this way (`init`, `refine`, `execute`, `review`, `autopilot`; `shared/` is a cross-referenced helper file, not a sixth skill). Confirm the body itself references `skills/shared/human-presentation.md` "relative to the plugin root" and still reads sensibly — a translated skill prepends one line naming the plugin's own absolute root so that reference resolves | `unknown command` means this regressed to the pre-item state. The skill runs but never mentions where its own plugin root is means the cross-reference note was dropped |
| 5.7 | Agents — does `/spawn @worker` resolve? | **Expected to WORK now** (same board item): a fresh child starts running `agents/worker.md`'s own body as its system prompt. Confirm its own declared `tools: Read, Edit, Write, Bash, Grep, Glob` actually restricts what it can call — **the bar is that declared tool restrictions survive, never widen**: a name Claude Code declares that conway has no counterpart for must be DROPPED and named in `/plugin`'s own listing, never silently granted, and never silently expanded to "every tool" | It resolves but has MORE tools than `worker.md` declares (a widened restriction — the load-bearing safety failure), or a legitimately-declared tool it should have is missing with no `/plugin`-visible explanation naming it |
