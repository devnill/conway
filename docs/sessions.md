...
| Flag / call | Effect |
| --- | --- |
| `--session <id\|name>` | Use this exact id, creating it if it doesn't already exist. Also accepts an operator-chosen name: an **unclaimed** name creates the session and binds the name in one step, so a script does not have to create, parse an id out of the JSON, and then `conway sessions name`. Colliding with an existing id — or with a name already bound — is a usage error pointing you at `--resume` instead, never a silent overwrite and never a silent rebind that would lose the old session behind its own name. One-shot only. |
