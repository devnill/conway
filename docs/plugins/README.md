...
| [`idiom.md`](idiom.md) — `conway.idiom` | What does the prepended conway-idioms instruction fragment actually say, where does it land relative to an agent def's own system prompt, and what decides which of its conditional sentences a given session — root, forked, or spawned — actually gets? | You want a bare interactive session to carry any harness orientation at all, or you're evaluating what this fragment assumes about which tools are announced. |
...
| [`inference-hooks.md`](inference-hooks.md) — hooks judged by a model (ABANDONED) | Why was a hook that judges a tool call by calling a model abandoned, and what does that leave open? | **Retired design record, not a plan** — the one named use case failed a real test, and not for a reason more model scale fixes. Read it before proposing an inference-evaluated hook again. |
