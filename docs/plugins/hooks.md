...
| Status | **designed-not-built, with the config surface now parsed and validated (still not dispatched).** `Plugin::manifest()`/`Plugin::tools()` is still the whole `Plugin` trait; there is no `hooks()` method, no `subagent_mode` field, and no `hook.fork` capability anywhere in the tree. A `[hooks]` block in `settings.json` now DOES exist as a typed shape (`conway::config::schema::HooksConfig`/`HookEntry` — `id`/`event`/`command`/`timeout_ms`/`enabled`, board item 01KZRZW5CWMVQ0GPRT4GX4RV5G) and is loaded, deny-unknown-fields-strict, and validated (non-empty/unique `id`) by every `conway::config::load` call — but nothing reads `HooksConfig::rules` to do anything with them: no dispatch path, no process spawn, no event firing exists yet. An operator who writes a rule today gets a config that parses correctly and then does nothing, by design and by label (GP-14) — see the two board items that close that gap: `01KZRZY1MNM872BZ6AKEBG3SKE` (the script runner that will actually spawn `command` when `event` fires) and `01KZS00JP5QNBJSSHNFP9C47GM` (`pre_tool_use` enforcement, the first concrete event the runner will be consulted from). Both are still tracked under the umbrella `01KZDC0RDRMMMJHX7SAFMM2Q5A`. **A script runner is not a second extension mechanism (GP-03):** the design is explicit that a script-dispatching hook is itself an ordinary `Plugin` whose own implementation happens to shell out per event, layered on top of the one mechanism, never beside it |
