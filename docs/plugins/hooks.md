...
| Ordering | Exactly one `ContextHook` per embedder (`ConwayBuilder::with_context_hook` takes one `Arc<dyn ContextHook>`, not a `Vec`) — but composition across *multiple* hooks IS reachable as of board item `01KZRZZP6A4A27R3EN0HQAENBS`: **Rust hooks chain in install order** (each hook's `before_request` output feeds the next, via `ChainedContextHook` in `crates/conway/src/builder.rs`), and a configured `request_assembled` script hook (point 13) is evaluated against the POST-Rust-chain payload — the output of the last Rust hook's `before_request` — and composes under §16.3's union rule (exclusions union, appends concatenate) rather than chaining. A script hook never sees another hook's edits, Rust or script; it sees only the post-Rust-chain payload, and its own deltas are applied afterward. Two or more script hooks on the same event compose identically (each sees the same post-Rust-chain payload). See point 13's own entry for the script-hook side |
...
| Ordering | One Rust hook, same as `before_request` -- but, as of board item `01KZRZZP6A4A27R3EN0HQAENBS`, composing with every configured `context_overflow` script hook (point 13): **the Rust hook runs first** (on a clone of the pre-edit payload), **then script hooks are evaluated against the PRE-Rust-hook payload** — the original, before `on_overflow` ran — and their deltas are applied to the Rust hook's output (`working`) by union (exclusions union, appends concatenate) rather than chaining. A script hook on `context_overflow` never sees the Rust hook's edits, unlike `request_assembled` where scripts DO see the post-Rust-chain payload. The runtime, not any hook, bounds the retry: `MAX_OVERFLOW_ATTEMPTS = 2` (`crates/conway-runtime/src/agent_loop.rs`) caps how many times `route_and_attempt` will call `on_overflow`/dispatch `context_overflow` for one turn — a hook that keeps returning a still-too-large payload cannot hang the turn |
