...
- **`ConwayBuilder::with_health_registry` and `ConwayBuilder::with_artifact_writer` — `SessionStore` gains the re-exports its own injection point needed** — board item `01M1WVQM440XZDSP0KC664HJ7S` (architecture review finding F11), per the operator's direct ruling: "build injection points for all three, same as other ports already allow." Three ports — `SessionStore`, `ArtifactWriter`, `HealthRegistry` — were the only ones an embedder could not reach the way every other port (`Router`, `PermissionGate`, `ContextHook`, `Backend`, ...) already allows, and two of the three had no stated reason why not (unlike `SubagentHost`, which carries an explicit, cited, single-authority ruling that is unchanged by this item). `ConwayBuilder::with_session_store` already existed; what it lacked was re-exports for `SessionStore::append`'s full signature — `LiveOwner` is now re-exported from `conway::plugin`, alongside the `SeqRange`/`StoreError` the port's narrower *calling* surface already needed. `with_health_registry` overrides the honestly degenerate default `AlwaysClosedHealthRegistry` (`EndpointId`/`Observation` now re-exported at the facade root, alongside the already-reachable `BreakerState`) — still superseded by an installed `RouterFactory`'s own `RouterBundle::health`, exactly as the default already was. `with_artifact_writer` overrides the runtime's own per-agent `AgentArtifactWriter` (confined to that agent's `cwd`/root) with a single, whole-runtime implementation for every agent — `ArtifactWriter::write` already takes the calling `AgentId` as a parameter, so a custom implementation branches on it directly rather than conway resolving or checking containment on its behalf; wired through a new `Runtime::set_artifact_writer`/`LoopDeps::artifact_writer`, the identical purely-additive `RwLock`-backed shape `Runtime::set_context_hook`/`LoopDeps::context_hook` already established for the same "no `RuntimeDeps` field to source this from" constraint. Calling none of the three preserves today's behavior exactly. See `docs/embedding.md`'s "What's reachable from the library, and what isn't" table, and `crates/conway/tests/session_store_injection.rs`/`health_registry_injection.rs`/`artifact_writer_injection.rs` for a facade-only, non-default implementation of each, checked to be genuinely called.
...
- **One-shot mode (`conway -p`) no longer sits silently for 90+ seconds against a slow or stalled backend** — board item `01M1WVK9PF5G57Y6R36B94S0RB`, a live-reproduced finding: a brand-new user's very first `conway -p "<prompt>"`, against a real local backend with no prior config, printed nothing at all — on stdout OR stderr — for over 90 seconds, with no way to tell it apart from a hang; `--output-format jsonl` against the identical invocation streamed real events within milliseconds. Two changes, both scoped to one-shot mode: (1) `text` output mode (the default) now prints a periodic `waiting for a response... <n>s` notice on stderr while a turn has not yet produced any visible reply text — silenced permanently the moment real text starts streaming; `jsonl`/`json` are unaffected, since both already show activity immediately. (2) One-shot now applies its own default wall-clock ceiling of **300 seconds** whenever nothing else already bounds the run (no `--max-seconds`, and no non-zero `[limits].deadline_secs` configured) — a stalled/crashed backend now fails loud with a named, actionable error (which backend, how long it waited) and exit code 5, instead of hanging indefinitely. **This changes out-of-box behavior for every existing one-shot user**: a run that previously waited forever now fails after 5 minutes by default if nothing ever responds. `[limits].deadline_secs`'s own config-wide default is unchanged (still `0`/unbounded) — this is deliberately a one-shot-only default, not a change to the interactive TUI's or any other caller's behavior, since `deadline_secs` is a session-lifetime ceiling that would otherwise end a long-running interactive conversation mid-chat. An operator who wants genuinely unbounded one-shot behavior should set `[limits].deadline_secs` (or `--max-seconds`) to a large-but-nonzero value, since a literal `0` is indistinguishable, after config merging, from never having set it at all. See `docs/scripting.md`'s "Budget flags" section and `docs/agents.md`'s `[limits]` reference for the full account.
...
- **A single tool result too large for the window is refused, not truncated — and headroom now scales to the model by default** — board item `01M1AVZPTRSWVE33G4DTJY7Q1B`, per the operator's 2026-09-01 ruling ("if the window is full, we shouldn't augment the request; instead, we should refuse with details"). This item's own walked session hit a genuine structural gap, independent of any one ceiling being wrong: conway has no recovery path when a conversation genuinely grows past a window, and compaction is deliberately never the core's own answer (`PHILOSOPHY.md` §5/§6: "a loud, typed refusal beats a clever recovery"). Two structural moves, no compaction:
  1. **Tool-result admission gate.** A single tool result larger than a configured bound is not rendered into context at all — `conway_core::capabilities::ToolResultBoundPolicy` resolves a per-role bound (default ON: 20% of the role's smallest reachable window, `routing.tool_result_bound_fraction`, capped at 8192 tokens, `routing.tool_result_bound_cap_tokens`; `0` at either the global or per-role level means unlimited), and `conway_runtime::context::builder::ContextBuilder`'s new `admit_tool_result` — the ONE seam both a session's own tool results (`own_segment`) and a fork child's INHERITED ones (`record_role_and_content`) render through — replaces an oversized result's content with a short note (size in bytes and estimated tokens against the bound, where the full result is kept — this session's own durable log, no new port — and two remedies: re-invoke the tool narrower, or fork a child to distill it) rather than the real content. Never a lossy middle: the whole result, or a note. `ContextReport` gains `not_admitted` (mirrors `dropped`'s "the intervention is part of the record, not behind it" discipline), surfaced in the TUI's `/context`. `RoutingError::ContextTooLarge`'s message now also names `/conway.history.rewind` and `conway.trim` as remedies for the case none of its existing three levers helps because the conversation itself has outgrown every candidate.
  2. **Adaptive headroom, finished.** `routing.headroom_fraction` (an opt-in adaptive fraction of a role's smallest reachable window, landed in a prior commit) is now ON BY DEFAULT (`d = 10`, i.e. 10%) rather than requiring an operator to discover and set it — the fixed `default_headroom_tokens` constant is now the fallback only when no model window is known at all, not the everyday case. An explicit per-role `headroom_tokens` still wins unconditionally; `headroom_fraction: 0` opts back out. Not clamping: this is a config-load-time computation over `[routing]`/model metadata, never a per-request runtime shrink — `headroom_exceeding_smallest_reachable_context_warns_without_clamping` (`crates/conway/tests/config_headroom.rs`) still pins the deliberately-rejected runtime-clamping alternative.

  Explicitly NOT built (the operator's own scope line): no default curator, `conway.trim` stays opt-in, the `ContextHook::on_overflow` seam stays empty by default — an operator who wants compaction installs a plugin; the core never defaults into it. See `docs/routing.md`'s "Adaptive headroom" section and "What conway does as a window fills" for the full account, including the two strategies that remain deliberately unbuilt.

- **Setup-time context window: conway now asks when discovery fails, and persists the answer — the second half of the operator's 2026-08-30 ruling** — board item `01M1DEBDFWP3X6DM7RP52M5ZTQ` ("at provider setup, we should attempt to determine the size. If its not available, we can ask the user to set it."). A prior item shipped DISCOVER only and disclosed the rest rather than faking it (`docs/providers.md`'s "Establishing the window at setup"). Both setup entrances — guided first-run and `/settings` → providers → add — now run the identical discover-then-ask-then-persist sequence through one shared set of primitives: `conway_plugin_backends::probe::discover_context_window` (DISCOVER, unchanged), `crate::first_run::ask_and_persist_context_window`/the settings TUI's new `Mode::AddProviderContextWindow` card (ASK — a plain "enter tokens, or leave blank to skip" prompt, opened only when discovery found nothing AND the dialect's own baseline is not already a real, sourced figure — `crate::first_run::context_window_is_verified` reads the SAME `context_window_verified` flag `capabilities::build_capabilities` itself consults, so Anthropic/OpenAI setups are never asked a question they already have a real answer to), and `crate::first_run::persist_context_window`/`persist_context_window_at` → `conway::config::metadata_path_for` → `conway::config::set_context_window` (PERSIST, new). **The file/scope decision**: the window is written into whichever `models.json` the CURRENT process would actually read back for its own `cwd` — the same `[models].metadata_path` resolution (default `.conway/models.json`, relative to `cwd`, unless a `user`/`project`/`env` layer overrides it) a normal `load` performs, computed without requiring a full config to already exist. Two alternatives were built, found unsafe, and reverted rather than shipped: always writing a FIXED user-scope `models.json` and pointing `metadata_path` at it (rejected — it would silently shadow an operator's existing project-local `models.json` the first time this code ran); and `backends.<id>.models.<model>.max_context_tokens` (confirmed INERT for `openai-compat` backends — `factory.rs`'s `ctx.extra` overlay is exercised for the `anthropic` kind only). **No invented numbers anywhere in this path** — a decline (blank `Enter` or `Esc`) is a real, honest answer, never an error, and a config with no recorded window — whether pre-existing or freshly declined — keeps resolving `ContextTokensSource::Unverified` exactly as it always has; this item does not touch that state, it only makes it rarer going forward. `finish_setup`'s wrong-credential retry path (`retry_credential_and_finish`) previously attempted no discovery at all — closed, not merely disclosed: it now runs the same ask-or-discover step before its own retried write. See `docs/providers.md`'s "Establishing the window at setup" for the full account.

### Removed

- **`Plugin::permission_modes` and `PluginDeclaredMode` are gone** — harness gap review 2026-09-01 finding 9. Operator ruling: "accept the recommendation in full (delete `permission_modes`, label `optional_host_caps`, keep `narrowable_keys` and the five single-implementor methods)." The method had zero production implementors — only a test fixture — for its entire life: it was built and wired end to end (board item `01M0X4YDNVP7TZ0PVSRJ0388SS`) for exactly one designed-for consumer, `conway.permissions`, a `pre_tool_use` hook that judged a tool call by calling a local model, and that consumer was cancelled outright on 2026-08-27 (decision record `01M128AP39WXE01BBZV4RENC4M`) after a 48-case corpus test showed a local model missing the paradigm case — telling a scratch `git reset --hard` from a real one via `cwd` — 100% of the time at both tested sizes. With the one consumer gone and no other plugin ever declaring a mode, the mechanism was a seam built for a hypothetical that did not materialize (INTENT.md §8.5), so it is deleted rather than kept "just in case," the same "a hook that silently never runs is worse than an absent one" reasoning `Plugin::on_init`'s own removal already established. Removed along with the method: `PluginDeclaredMode`, the `conway-core::mode_cycle` module in full (`ModeCycle`/`ModeCycleEntry`/`DeclaredModeRef`/`DeclaredModeCollision`), `PermissionBroker`'s `active_declared_mode`/`set_active_declared_mode`/`select_mode_cycle_entry`, `Conway::mode_cycle`/`active_declared_mode`/`current_mode_cycle_entry`/`cycle_permission_mode`, the TUI's `DeclaredModeMirror`/`AppState::declared_modes`/`AppState::active_declared_mode` and their status-line rendering, and `crates/conway/tests/declared_permission_modes.rs`. **Safety-critical resolution logic stays single-implementation and untouched: `PermissionBroker::decide` reads the exact same `mode: RwLock<PermissionMode>` field it always has, and `Action::CyclePermissionMode`'s Shift+Tab handler reverts to the identical `Prompt -> Plan -> AutoAllow -> Prompt` inline cycle it used before the declared-mode feature existed** — verified by reading `decide()`'s own match before and after this change, which never referenced `active_declared_mode` at all. `docs/plugins/permission-modes.md` is retired into an ABANDONED design record (modelled on `docs/plugins/inference-hooks.md`), kept and still linked from `docs/plugins/README.md` with its row reworded; `docs/vision/DESIGN-permission-modes.md` gets a correction note in place rather than a silent edit. `PluginManifest::optional_host_caps` is a different row of the same ruling and is **kept, labeled** rather than deleted: it is carried on the wire and honoured (degrade-and-announce) by every consumer that already exists, with no first-party producer today — labeled at the field's own doc, `crates/conway/src/host_caps.rs`'s module doc, `docs/plugins/subprocess-plugins.md`'s wire-points bullet, `docs/plugins/hooks.md` point 1, and `conway-plugin-subprocess`'s own module doc, plus a new `scripts/board-claims.md` `absent:` predicate over every `crates/conway-plugin-*/src` for a non-empty `optional_host_caps: vec![` literal, so the label fails the day a producer ships. `narrowable_keys` (one production implementor, `conway.fs`) and the five single-implementor methods (`observers`, `curators`, `permission_rules`, `observe_sink`, `hooks`) are unchanged by this item — the ruling's own "keep" rows, documentation-only defects owned elsewhere.

### Fixed

- **A terminated agent now reports what it actually did — a budget, cancellation, deadline, or backend-failure termination no longer discards the agent's own account of its work down to a bare status name** — board item `01M1FH114QA3A152W8H6E2YMGJ`, dogfooded on this repository: an agent doing real, disk-visible work (69 lines across three files) hit a subagent's default 40-step budget mid-task and reported `(no output; terminal status: budget_exceeded)` — the uncommitted work survived only because it had already been written to disk, and the operator found it by hand. `AgentLoop::check_budget` (all four `[limits]` dimensions), `finish_cancelled` (both graceful and immediate cancellation), the deadline arms in the resume-prompt gate and mid-routing race, and both arms of `finish_error` (a bubbled-up backend/store failure) all used to call `finish(status, "", ...)` — ten call sites, one literal `""` each; `grep -n 'finish(' agent_loop.rs` now finds none. `LoopState` gains `last_assistant_text`, overwritten every turn a backend response lands (BEFORE that turn's own tool-dispatch/cancel/budget checks, so a termination mid-tool-batch still sees the text that accompanied those calls) — no longer `Copy` as a result, and `check_budget`/`finish_cancelled`/`finish_error` now take `&LoopState` rather than an owned one. Every one of the ten sites now calls a new `AgentLoop::terminal_account`, which resolves, in order: the captured text if there is any; failing that, an explicit "stopped mid-run" marker naming the turn/tool-call counts when there is other evidence of real work (`state.turn > 0`, `state.tool_calls > 0`, or a new `ResultBuilder::has_activity` — a tool artifact or an observed `report` call) — acceptance criterion 2, "distinguishes 'no work' from 'stopped before reporting'"; failing THAT, the unchanged `""`, correctly reserved for a termination before this agent's very first turn ever produced anything. `finish_error`'s own doc previously accepted losing the run's summary fidelity on a late I/O error as a deliberate trade-off — that acceptance is withdrawn: the `(RuntimeError, LoopState)` error tuple every `try_rt!` site already threads out of `run_inner` carries `last_assistant_text` regardless of which turn the failure hit. Tested end to end against the real compiled loop, not the unit level: `crates/conway-runtime/tests/agent_loop_e2e.rs` (each budget dimension, a backend failure after a successful turn) and `crates/conway-runtime/tests/steering.rs` (hard and soft cancellation) all assert the terminal `AgentResult::summary` directly. The step-count-vs-limit-scale confusion the same incident's record also named (`steps_taken: 81` beside `limit: max_steps=40`, reading as a 2x overrun when `max_steps` is turn-scoped and `steps_taken` is session-lifetime for a `keep_alive` agent) is a separate, sibling board item's scope, not this one's.
- **"Zero percent cached" and "this provider doesn't report caching" are no longer the same reading — a nine-million-token session with a provider that never sends a cache field now says so** — board item `01M1FSGJM3HAT3493ZRQ2Z4AP0`. `Usage` gains `cache_accounting: CacheAccounting` (`Reported`/`NotReported`, default `Reported` — old logs without this field decode as `Reported`, which for pre-existing zero-filled Ollama-native records renders as `0% cached`, not `not reported`: honest enough for a field that did not exist yet). `NotReported` is sticky under `Add`/`AddAssign` — a session that mixes a cache-reporting turn with a non-reporting one cannot honestly claim its aggregate percentage is real. Ollama's native `/api/chat` path (`crates/conway-plugin-backends/src/openai_compat/ollama_native.rs`) always sets `NotReported` — its wire format has no cache field of any kind, streaming or not. The OpenAI-compatible and Anthropic wire decoders (`openai_compat/wire.rs`, `anthropic/wire.rs`) set `Reported` only when the response actually carried a cache field (present, even if zero) and `NotReported` when it did not — the OpenAI-compatible dialect already declared `cached_tokens` `Option<u32>`; Anthropic's `usage.cache_read_input_tokens`/`cache_creation_input_tokens` are always present once caching went GA, so `Reported` there tracks "was a `usage` object sent at all," not per-field presence. The streaming drive loops in `openai_compat/stream.rs`/`anthropic/stream.rs` seed their running `Usage` as `NotReported` up front (not `Usage::default()`, whose `cache_accounting` now defaults to `Reported`) — a dialect that streams a whole response with zero `usage` frames (any `supports_stream_options = false` dialect) must not have that silence persisted as "reported: zero." Every TUI renderer that shows a cache percentage (the status line's `tokens` field, the turn-end summary line) now shares one rule via `conway_cli::tui::usage_format::cache_suffix`: `Reported` ALWAYS shows a percentage, including a genuine `0% cached` (previously omitted, which is exactly the bug); `NotReported` shows `cache: not reported by <backend>` (or the generic `cache: not reported` when the backend id isn't at hand) instead of a percentage that would misrepresent an unobserved figure as a real zero. See `docs/providers.md`'s Ollama native-endpoint section and `docs/interactive.md`'s `tokens` status-line field.
- **A dropped stream mid-reply no longer silently switches models or ends the turn — it retries the same one** — board item `01M1FSJ4E2S5M9KBSBJAAPJQ48`. Ordinary request failures already retried with backoff (`conway_plugin_backends::http::HttpClient::send_with_retry`, up to two retries), but that helper only ever covers a request's INITIAL response — a mid-stream `Transport`/`ServerError`, or a stream that ends without ever reaching a `Done` chunk, used to either advance the fallback chain immediately (changing which model answers your prompt, mid-task, with no warning) or, on the last candidate, fail the whole turn outright. `conway_runtime::attempt::AttemptEngine::execute`'s per-candidate loop now retries that SAME candidate up to twice more (three attempts total, sharing `send_with_retry`'s exact backoff policy via the new `conway_core::retry` module, so the two can never drift), discarding the partial reply VISIBLY rather than either of the old behaviors: a new `Event::StreamRestarted { agent_id, attempt, discarded_text_chars, discarded_thinking_chars }` fires before each retry, and every renderer handles it — the TUI truncates the in-progress reply/reasoning bubble back to before the discarded deltas and shows a "stream restarted (attempt *N*); partial output above discarded" notice, the one-shot `text` renderer prints a newline plus a matching stderr diagnostic (partial stdout text cannot be unprinted — a script that needs clean stdout should use `--output-format json` instead, per `docs/scripting.md`'s "Streaming" section), and `jsonl` carries it like any other event. The retry is eligible only once at least one real chunk has actually been read off the stream — a connection that opens and fails immediately with zero content (a proxy health-checking a dead upstream, an HTTP/2 reset right after headers) is indistinguishable from a pre-open failure and fails over immediately, exactly as before this item. Each failed attempt, retried or not, still records its own health `Observation` exactly as before; the fallback chain only advances once the same-candidate budget is exhausted. `RateLimit`, a `RequestIncompatible`/`Fatal`-class error, and any failure raised before real content arrived are untouched — those keep today's behavior exactly. See `docs/providers.md`'s new "Retries and backoff" section for the full policy.
...
- **A tripped turn budget on an interactive session now ends the turn, not the conversation** — board item `01M1FSP1QJFCHA7H8QPYZ9GG1P`, live-reproduced during dogfooding on this repository: a `keep_alive` root hit `steps_taken: 76` against `max_steps=40` with zero warning and the whole session died mid-task. `AgentLoop::check_budget` used to call `self.finish(ResultStatus::BudgetExceeded, ..)` for every exceeded dimension, ending the agent outright — for a `keep_alive` session that meant an ordinary per-turn runaway-tool-loop guard (`max_steps`, `max_tool_calls`) silently killed the whole interactive conversation. Those two dimensions are turn-scoped for a `keep_alive` agent (`LoopState::turn_steps`/`tool_calls`, reset at each user-turn boundary) — `check_budget` now trips them into a new outcome, `BudgetCheck::TurnAborted`, instead: a `LogRecord::SystemNote { reason: "budget_turn_aborted" }` tells the model why in its own words ("this turn was ended by the harness: … reached (*n* steps this turn). Answer with what you have; the operator can continue with a new prompt."), a new `Event::TurnAborted { agent_id, limit, steps_this_turn }` tells every subscriber, and the SAME turn-boundary reset a natural completion performs runs again — factored into one shared `AgentLoop::end_keep_alive_turn`, so there is exactly one implementation of "end a keep_alive turn cleanly" — before the loop returns to idling for the next prompt. `deadline`/`max_tokens` (session-lifetime, unchanged) and every non-`keep_alive` agent (byte-identical to before this item) still end the whole run via `BudgetCheck::Finished` exactly as before. The TUI renders the live event as a transcript notice (`turn ended: max_steps=40 reached; type to continue`); `jsonl` carries it like any other event; the one-shot `text` renderer gets a matching stderr line, though a one-shot run is never `keep_alive` in practice. See `docs/interactive.md`'s new "When a turn is cut off" section and `docs/scripting.md`'s "Budget flags" section for what changed for an operator/embedder.

### Added

- **A hung tool can no longer hold a turn forever — an operator-configurable per-call timeout now runs at the one seam every tool call goes through** — board item `01M1FSHJ3FG522MHA9CMBJTVW1`. New `[limits].tool_timeout_secs` (`u64`, `0` = unlimited, the default — byte-identical behavior for every operator who never sets it) reaches `conway_runtime::tools::ToolBatchCtx::tool_timeout: Option<Duration>`, and `ToolRunner::run_batch`'s per-call dispatch now enforces it: past this deadline, the call's cancellation token is cancelled (so `bash` kills its process group, and any cooperative tool watching `ToolCtx::cancel` stops on its own) and the call returns a model-visible error naming the tool and the limit, rather than the turn hanging until a person cancels it by hand. Implemented by spawning `Tool::invoke` as its own task rather than racing it inline: a plain `tokio::select!`/`tokio::time::timeout` around an inline future drops the loser the instant the timer wins, which would abandon a cooperative tool's own future before it ever got a further poll to notice the cancellation signal — spawned, the task keeps running (and keeps being polled) after the runner stops waiting on it, so a well-behaved tool genuinely gets the chance to react on its own time. `[limits].tool_timeout_secs` is distinct from `bash`'s own pre-existing per-call `timeout_ms` argument (unchanged by this item) — when both apply to the same `bash` call, the smaller one wins; see `docs/tools.md`'s new "Timeouts" section. **Disclosed gap, not solved here**: `conway_runtime::agent_loop::AgentSpec` has no field to source this from yet (mirroring `[limits].max_parallel_tools`'s own already-disclosed gap, `conway::builder`'s module doc), so a real session built through the facade still passes `None` unconditionally regardless of a configured `tool_timeout_secs` — this item lands the config key, its parity with `LimitsConfig::default()`, and the runner's own enforcement and its test coverage; threading it from a real `settings.json` through `RootSpec`/`AgentSpec` to a live session is a follow-up.
- **`/context` now leads with a summary header instead of making you count lines by hand** — a real session had 183 segments and 175k estimated tokens, with no total shown anywhere and no way to see what was taking the space short of scrolling the whole listing. `/context` now prints, ahead of the unchanged per-segment listing: a header line (`context: 175,212 tok est across 183 segments`), a by-kind table ordered by tokens descending — every provenance variant grouped on the variant alone, `ToolResult` further split by `tool` name so `tool result: read` and `tool result: bash` never collapse into one misleadingly large row — and a `largest:` block naming the five single biggest segments and the tool/call id that produced each, so a runaway file read or oversized tool result is visible immediately. Computed by a new pure function, `conway_cli::tui::commands::summarize_context_report(&ContextReport) -> ContextSummary` (`pub(crate)`, no `AppState`/facade/TUI type anywhere in it), reachable from anything holding a `ContextReport` — not only this renderer — so a future one-shot JSON context dump could report the identical numbers without a second implementation. Grouping keys on the `Provenance` variant itself, never on a `SystemNote`'s `reason` text, so a sibling item changing what reason gets stamped on an assistant turn cannot silently fragment the table. An empty context still shows only the plain `empty context` notice it always has. See `docs/interactive.md`'s `/context` section for the full example.
- **The model is now warned before a budget or the context window trips, not only after** — real dogfooding incident: a long-running `keep_alive` session silently hit its step budget mid-real-work, with zero warning, and the operator lost the ability to continue a real conversation with no idea why. conway computed how full the context window was and how close each budget dimension was to tripping every turn already, but told only the human (`/context`, the status line). `conway_runtime::runway` (`crates/conway-runtime/src/runway.rs`, wired into `AgentLoop::run_inner`) now tells the model too: a `SystemNote { reason: "runway" }` once per session when the routed model's context window crosses 50%, 75%, or 90% full ("fork the remaining exploration to a child and keep only its distillate"), and once per turn when a configured `max_steps`/`max_tool_calls`/`max_tokens`/deadline comes within 20% of its limit ("wrap up or report now") — each threshold fires at most once, and the turn-scoped budget dimensions reset at a `keep_alive` session's next user turn. No note when the routed model's context window is not actually known (say nothing rather than guess), and no change to what trips a budget — this is purely an earlier, additional notice. Reaches both renderers for free: `LogRecord::SystemNote` is the exact mechanism `conway.stepguard`'s repeated-call note already uses, so the TUI transcript and `--output-format jsonl` both already know how to show it. `conway.idiom`'s fragment now states this promise in the same change that makes it true (`crates/conway-plugin-idiom/fragments/idiom.md`); `docs/plugins/idiom.md`, `docs/interactive.md` ("Runway notices"), and `docs/scripting.md` ("Budget flags") describe what it looks like.
- **`/await <agent>` — the operator's counterpart to the model's `conway_await` tool** — board item `01M1FRWHMSB4XC694KH715RASG`, INTENT.md §7a parity: `/fork`/`/spawn`/`/steer`/`/cancel` already let the person at the keyboard drive the session's agents the same way the model can, but there was no way to ask "tell me when that one is done" without staying focused on it. `/await` posts an immediate notice ("awaiting `<agent>`; a keep_alive agent ends only on `/cancel`") and returns control right away — the input loop stays fully responsive — then posts a second notice once the target reaches a terminal state, naming its status, its summary, and how many facts/artifacts it produced. Awaiting the session's own root agent is refused (use `/quit`); a second `/await` on an agent already being awaited is refused too — one operator-side waiter per agent. `commands::execute`'s `SlashCommand::Await` arm never calls the new `Host::await_agent` (a thin passthrough to `SessionHandle::await_agent`, the same facade primitive the model-facing tool reduces to) itself — it only validates and returns `Effect::RunAwait`, mirroring `/ask`'s own `Effect::RunModalAsk` shape exactly, so a `keep_alive` agent that runs indefinitely can never freeze the TUI (`commands::tests::execute_never_awaits_a_hanging_await`, demonstrated failing first against a scratch version that awaited inline). `App::spawn_await` does the actual `tokio::spawn`, over its own dedicated channel (`app/await_cmd.rs`) rather than `/ask`'s `modal_ask_tx` or the session's `AgentFinished` event stream — see that module's own doc for why both alternatives risk losing the completion notice when the operator has since focused elsewhere or the event stream gets resubscribed. See `docs/interactive.md`'s slash-command table and agent-panel section.
- **conway now warns at startup when `--root` is set alongside `bash` — the one built-in tool a confinement root cannot actually confine** — harness gap review 2026-09-01, finding 10. An operator who sets `--root`/`ConwayBuilder::with_root` reasonably believes nothing can touch files outside it; before this item, the only place that said otherwise was prose in three documents. `ConwayBuilder::build` now checks, after every plugin's final installed tool set is known: if a root is set AND any registered tool declares both `PathArgs::Unconfinable` and `RenderKind::ShellCommand` (today, only `bash`/`conway.shell` — computed from that structural pair, never from a hardcoded tool-name check, so a future tool making the identical two claims about itself earns the identical warning automatically), it pushes exactly one `ConfigWarning` (`WarningCode::RootWithUnconfinableTool`) naming both `bash` and `--root`, surfaced on the same channel every other build-time warning already uses (`conway-cli`'s `diag::warn` on stderr for every non-interactive dispatch target, the TUI's own startup transcript otherwise). Does **not** refuse to start, auto-exclude `bash`, or inspect command text — the warning is advisory, matching `--root`'s own existing "confines path arguments, not what a shell command does" behavior exactly. `ConwayBuilder::with_root`'s own doc comment, `docs/permissions.md`'s Confinement section, and `docs/tools.md`'s shell section each now name the warning instead of leaving it as prose alone.
- **`conway tools list`: a headless subcommand that prints every registered tool, so `--allowed-tools`/`--deny-tools` scripting has a self-describing catalog outside the interactive screen** — harness gap review 2026-09-01, move 10. `Conway::tool_specs`/`Runtime::tool_specs` (new facade/runtime capability, delegating to `PluginRegistry::specs(None)`) is the one read surface for "what is registered"; `conway tools list` renders it as `name  category  permission-class  first-sentence-of-description`, one line per tool sorted by name, plus a trailing `N tools registered from M plugins` count line (`Conway::tool_plugin_count`, distinct plugin ids that registered at least one tool); `--json` prints the full `ToolSpec` list (name, description, JSON Schema, category, permission class) as one JSON array. Under `--root`, the list itself is unchanged (announcement and confinement are separate questions) and gains one trailing footnote naming which listed tools are path-confinable (`Conway::tool_path_args`, `PathArgs::Named`). **Disclosed deviation from this item's own spec**: its acceptance criteria asked for a test toggling `tools.builtin_plugins`'s `"conway.shell"` entry and observing `bash` appear/disappear from this command's output — `crates/conway-cli/src/main.rs`'s `build_conway` already forces `PluginSelection::All` for every non-interactive CLI dispatch target (`sessions`, `routes`, one-shot `-p`, and now `tools`), exactly mirroring `docs/getting-started.md`'s existing "Enabling bash" disclosure that one-shot mode "is unaffected by this default" — so `conway tools list` shows `bash` unconditionally, same as a script's own `-p --allowed-tools bash` invocation would actually get; a config-derived answer that disagreed would be a declaration-honesty violation against the exact audience ("self-describing allow-list") this item serves. `crates/conway-cli/tests/tools_list.rs` proves this is deliberate (`tools_builtin_plugins_toggle_has_no_effect_on_bash_registration`) and separately proves a config value that DOES change the registered set (`[plugins].install`, honored identically by every dispatch target) with a real observed difference (`plugins_install_toggle_changes_the_registered_tool_set`). `docs/tools.md`'s "Which tools are actually registered" and `docs/scripting.md`'s flag reference both point to it now.
...
- **Guided first-run setup now installs conway's own opinion set and asks a plain yes/no about the shell tool, instead of leaving a fresh operator with a model that has no system prompt, no loop guard, no memory, and no shell — and no notice that any of that is true** — board item `01M1FS34GNZEVZP4ZBVC90VD6J`, decision `01M1FQFP5D0R3M9GC8R8Z24F5N` (2026-09-01). The moment guided setup (`crates/conway-cli/src/first_run.rs`) verifies a working provider, it now writes `first_party_plugins::DEFAULT_OPINION_SET` — `conway.idiom`, `conway.stepguard`, `conway.skills`, `conway.memory`, `conway.names`, `conway.history` — into `plugins.install` (`apply_opinion_set`, one `set_plugin_installed` call per id, idempotent), prints a table naming each id and its own `PluginDescription.summary`, and states exactly one removal sentence naming both routes (`conway plugin remove <id>` and hand-editing `plugins.install`). It then asks `Enable the bash shell tool? [y/N]` with a two-line caveat (bash is **not** confined by `--root`; every call still passes the permission prompt) — `y` writes `tools.builtin_plugins` to the harness's own preset list plus `"conway.shell"` (`apply_shell_choice`, via a new writer, `conway::config::set_builtin_plugins`); anything else, including `Esc`, writes nothing at all — the file is left byte-identical, since `tools.builtin_plugins`'s own absence already means the unmodified harness default. **This changes only the settings *file* guided first-run writes — it touches neither `ToolsConfig::default()`/`PluginsConfig::default()` in `crates/conway/src/config/schema.rs` nor `config::merge::default_document`, both of which stay exactly as they were**: a library embedder, a hand-authored `settings.json`, and the harness's own `ConwayBuilder::build()` are all unaffected by this item. `docs/getting-started.md`'s "Installing a first-party plugin" example now shows the six-id default (previously `conway.plugin_skeleton`/`conway.history`/`conway.stepguard`, an example that undersold and misdescribed what first-run actually leaves behind) and "Your first session" states what a fresh first-run installed and why; `README.md`'s "First-party plugins" section now distinguishes the harness (still registers nothing on its own) from the binary's first-run set; `docs/plugins/idiom.md` cross-links the new default; `PHILOSOPHY.md`'s stale "the binary does not yet turn on anything beyond the harness's own default set" note is deleted in its own commit, separate from this behavior change.

- **`conway.confine`: a bash-equivalent tool whose every command runs inside the operating system's own containment primitive, so `--root` is a real write boundary rather than a path-argument convention `bash` has always been outside of** — board item `01M1FSK1PSAJRG5N1E9N67QF0J`, harness gap review 2026-09-01, decision `01M1FQG08GDQ71984T0W0RJ019`. New first-party plugin, `crates/conway-plugin-confine` (`conway.confine`, one tool: `confined_bash`), installed via `[plugins].install = ["conway.confine"]` like any other first-party plugin. Its launcher wraps the identical `/bin/bash -c <command>` `conway.shell`'s own `bash` runs bare, in `sandbox-exec` on macOS (`(deny file-write*)` then `(allow file-write* (subpath "<root>"))`, last-matching-rule-wins) or `bwrap` on Linux (`--ro-bind / /` then `--bind <root> <root>`); **reads and network are not confined — only writes**, per the ruling. Mechanism, not policy: neither launcher ever reads `command` to decide anything — the OS primitive gets the whole string verbatim, so there is no Rust-side classification logic that could drift from what the OS actually enforces. Reuses `bash`'s own streaming/cancellation/timeout run loop rather than a second implementation: `conway_tools::shell::bash::BashTool` gained a pluggable `Launcher` seam (`BashTool::with_launcher`, re-exported as `conway::plugin::{BashTool, Launcher}`), and `ConfinedBashTool::invoke` resolves this agent's `conway.fs.root` (the SAME root `conway.fs` itself enforces, read from the identical per-agent config key, so the two tools can never disagree about the boundary) and delegates to a freshly-built `BashTool::with_launcher` instance. No root configured for an agent, or the primitive missing at plugin construction (`ConwayBuilder::build` config error naming the binary) or by call time, are all refusals — never a silent fall-through to running unconfined. `Tool::confined_by_tool` is a new trait method (default `false`) `confined_bash` answers `true`: the root+unconfinable-shell-tool startup warning (`WarningCode::RootWithUnconfinableTool`) now consults it and no longer fires for a tool that is, itself, the remedy — its own message now names `conway.confine` as the alternative to dropping `conway.shell`. Guided first-run setup detects a present primitive and offers `confined_bash` FIRST, ahead of the plain-bash question, when one is found (`apply_confine_choice`, `crates/conway-cli/src/first_run.rs`). `docs/plugins/confine.md` is the dedicated page (what is and is not confined, the exact per-OS invocation, and which OS the guarantee is verified on in this tree — macOS, by an in-crate test; the Linux `bwrap` mirror is implemented but not independently exercised by this tree's own CI); `docs/tools.md`, `docs/permissions.md`, and `docs/plugins/trust-and-security.md` (two sections) all cross-reference it; `docs/plugins/README.md`/`README.md` counts move to twelve/nineteen respectively.
...
- **`conway plugin list|install|remove` — the interactive `/plugin` command's table and toggle, headless — plus a startup notice when a config has never named `plugins.install` at all** — board item `01M1FSDRF20E2EGHCG3RK28DKH`. `cli::Command::Plugin`, a built-in clap subcommand matched ahead of the plugin-contributed-subcommand catch-all (`Command::External`): `list` prints one row per compiled-in first-party plugin (`[x]`/`[ ]`, id, summary), reusing `/plugin`'s own row builder (`rows_from_plugin_browser`, lifted out of `tui::view::plugins` into a new ratatui-free `plugin_rows` module so the headless and TUI paths call the identical function rather than two independently-worded listings) — `--verbose`, or `list <id>`, also prints the "you get / you lose / costs" breakdown. `install <id>...`/`install --defaults` (writes `first_party_plugins::DEFAULT_OPINION_SET`, the same six ids guided first-run setup installs) and `remove <id>...` call the SAME `conway::config::set_plugin_installed` writer against the SAME file (`~/.conway/settings.json` or `$CONWAY_CONFIG_DIR/settings.json`) `/plugin`'s own compiled-in toggle writes — no project-scope target exists for this command, by design. An id this binary does not link is a usage error (exit 2) naming every id it does; `conway plugin` alone prints help and exits 2. Separately, `first_party_plugins::install` (the one choke point every dispatch target's `build_conway` call already shares) now pushes a new `ConfigWarning` (`WarningCode::NoFirstPartyPluginsInstalled`) whenever the user-scope settings document names no `plugins.install` key AT ALL — absent, not an empty array, a distinction read off the raw document by a new `conway::config::plugin_install_key_present` helper (next to `set_plugin_installed`, no `PluginsConfig` serde change) rather than the merged, five-source config, which cannot tell "never mentioned" from "explicitly emptied." The message is built from `DEFAULT_OPINION_SET` itself, so it can never name a stale set. `docs/scripting.md` gets a new `conway plugin` section (list/install/remove reference, exit codes, the startup notice); `docs/getting-started.md`'s "Installing a first-party plugin" names the command in place of the old "(subcommand tracked on the board)" parenthetical; `docs/interactive.md`'s `/plugin` section cross-links the headless equivalent.

### Documented

- **The two-part bash ruling (decision `01M1FQG08GDQ71984T0W0RJ019`) is now recorded where an operator actually reads it, and the three unrelated Rust types all named `PermissionMode` are disambiguated** — board item `01M1FS46ZGP0FG6CTWPZP44P94`. `docs/permissions.md` gains a "Working with bash day to day" subsection right after the mode table, stating plainly: there is no durable bash grant and there will not be one (linking to Limits' own fuller account of why); session `AutoAllow` is the intended anti-fatigue lever, with what still applies inside it and how to enter/leave it; and `conway.confine` — a real, shipped first-party plugin as of this same cycle, not a forward declaration — is the guardrail that makes blanket approval honest, one sentence plus a link to [`docs/plugins/confine.md`](docs/plugins/confine.md). Limits' own "durable pattern grant does not exist for `bash`" bullet now points back to this subsection. `docs/tools.md`'s shell section cross-links it too. Separately, three Rust enums happened to share the name `PermissionMode`, one accident of naming with no relation to the doc gap above: `conway_cli::cli::PermissionMode` (one-shot mode's `--permission-mode` flag) is renamed `OneShotPermissionMode`; `conway::config::schema::PermissionMode` (`permissions.mode` for a library embedder with no gate of its own) is renamed `PermissionsConfigMode`; `conway_core::permission_mode::PermissionMode` (the TUI's operator-facing `Prompt`/`Plan`/`AutoAllow` mode, re-exported as `conway::PermissionMode`) keeps its name unchanged — it was never the ambiguous one, the other two were renamed around it. Rust identifiers only: every wire form (`--permission-mode allowlist`/`deny`, `permissions.mode = "prompt"/"allowlist"/"deny"`) is byte-for-byte unchanged, proven by `crates/conway-cli/tests/oneshot.rs::permission_mode_allowlist_wire_string_still_parses_after_the_rename` (drives the real compiled binary) and `crates/conway/tests/config_precedence.rs::permissions_mode_prompt_wire_string_still_parses_after_the_rename`. Neither renamed enum is re-exported at either facade crate's own root, so no `#[deprecated]` alias was needed for either rename.
- **`authoring.md` now covers every optional `Plugin` method that survived the consume-or-delete pass, not just the two or three this walkthrough happens to exercise** — board item `01M1FSKJHXMSTSM9076W5MR4KM`. The guide teaching people to write a plugin never mentioned `context_hooks`, `curators`, `permission_rules`, `capabilities`, `narrowable_keys`, or `observe_sink` at all, and its own "Writing a Rust plugin" section carried a paragraph, "Two new things on `Plugin` since this page was last written," that had drifted stale to the point of claiming `Plugin::hooks()` did not exist while the trait had shipped it. A new "What else a plugin can declare" section (after "Writing a Rust plugin") gives all thirteen optional methods a short, cross-checked subsection each: what it declares, the real consumer symbol that reads it (verified by `grep` against this tree — e.g. `ConwayBuilder::build` installs `permission_rules()` via `PermissionBroker::remember_deny_rule`/`remember_prompt_rule`), the smallest shipped first-party implementor to copy from, and how multiple plugins compose (union, chaining, or exclusive-refusal, read off `ConwayBuilder::build`'s own composition helpers — `compose_context_hooks`/`compose_curators` chain in install order; `capabilities()` refuses a duplicate provider name outright rather than picking one). Both stale paragraphs are corrected in place rather than left beside the new section to contradict it. A real facade parity gap surfaced along the way and is closed, not merely noted: `NarrowingRule` — the type `Plugin::narrowable_keys()` returns and a facade-only plugin author must construct — was missing from `conway::plugin`'s re-export list in `crates/conway/src/lib.rs`; it is exported now, and `docs/embedding.md`'s "Writing a plugin" section names it alongside the other per-method return types it was missing. `scripts/board-claims.md` gains six `present:` predicates, one per method that had zero prior mentions, so the guide cannot silently lose them again.
- **A subprocess plugin or an MCP server can bring only tools; the extension-point ceilings for both tiers are now labeled where each is described, not left for a plugin author to find by failing** — board item `01M1FRW1J1GV5VCNM22RJTRM15`. conway can load plugins written in other languages (`conway-plugin-subprocess`) and connect to MCP servers (`conway-plugin-mcp`), but neither route ever brings a system-prompt paragraph, a slash command, or an MCP prompt — a plugin author reading only the wire-point tables had no way to know that short of trying and failing. `docs/plugins/subprocess-plugins.md`'s "What this is not" section gains a fourth bullet, **Trait surface**, naming exactly which `Plugin` methods have no wire projection at all (`instructions`, `commands`, `events`, `hooks`, `curators`, `narrowable_keys`, `context_hooks`) — a subprocess plugin brings tools, a permission policy, an observer sink, status text and capabilities; nothing else — with the identical claim in `conway-plugin-subprocess`'s own module doc, next to the pre-existing "no `context.hook/1`" disclosure; `docs/plugins/hooks.md` point 17's Status row now says "in-process only" and names both out-of-process tiers as unable to reach it. `docs/plugins/mcp.md`'s "What it deliberately does not do" gains **No MCP prompts or resources**, naming `prompts/list`, `prompts/get`, `resources/list`, `resources/read` as never called (verified: `grep -rn 'prompts/\|resources/' crates/conway-plugin-mcp/src` matches nothing) and what a consumer would look like — a prompt as a namespaced slash command, a resource as a read tool or an instruction fragment — with the same disclosure in `conway-plugin-mcp`'s own module doc. Both labels are enforced, not merely asserted: `scripts/board-claims.md` gains two `absent:` predicates (`"instructions/1"` over `crates/conway-plugin-subprocess/src`, `(prompts/(list|get)|resources/(list|read))` over `crates/conway-plugin-mcp/src`, covering all four named methods), each failing the day the wire actually grows that call, forcing the label's deletion in the same change that builds the capability. Neither capability is built or scheduled by this item — no wire points, no MCP calls, no new config keys; ruling was to state the ceiling, per INTENT §8.5, not build a seam with no consumer.
- **Why a translated skill's slash command uses `.`, not the `:` Claude Code and `ideate`'s own docs show you — and the palette's live filter no longer goes blank when you type it that way** — dogfooding finding `01M1M3RKHNAJP1TRNTT3ENH68W`, filed during the capstone virgin-walk (board item `01M0X1GRJ52SF38FV8E0V7V7B4`). The operator installed `ideate` via `[plugins].claude_compat`, read its own docs and Claude Code's own skill listing (both name a skill `/ideate:refine`, a colon), and found conway's own palette showing `/ideate.refine` instead — confirmed as a real, working, but entirely undisclosed mismatch: conway's own slash-command namespace separator has always been `.` (`EVENT_NAMESPACE_SEPARATOR`), reused unmodified for a translated skill (board item `01M1DG5TTF6NHW2RXJRZ8ZPE7K`), and `commands::parse` already accepted a leading `:` as a typed-input alias for a FINISHED word, but the palette's own live filter (`view/palette.rs::matches`) did not apply the identical alias to a still-being-typed prefix — typing `/ideate:re` showed nothing at all, live, even though finishing the word and pressing Enter would have worked. `docs/plugins/claude-compat.md` gains a dedicated "Command naming: a Claude Code `:` becomes conway's own `.`" subsection stating the rule plainly and why. `commands::translate_colon_alias` is a new, shared one-line helper (`parse`'s own plugin-command arm now calls it too, rather than restating the same `split_once(':')` logic a second time) that `matches()` now runs the live input through before filtering, so a colon-typed prefix surfaces the SAME dot-named row a dot-typed prefix already did — the palette suggests the working form instead of looking empty.

- **"What is a role?" now has an answer where a first-time user reads it** — board item `01M1JJJYXRED38H2NY03PX8W4C`, found dogfooding: the operator hit `/settings` on a virgin walk and reported "I'm not sure what role means; it's a little ambiguous." `role` is a core routing concept (`roles.<alias>.chain`) named throughout `docs/getting-started.md` and every `settings.json` example, but the guide used it as if the reader already knew what it was. A first-time user configuring their first provider has no reason to. `docs/getting-started.md` now defines a role in one sentence the first time the word appears: a role names one purpose — coder, reviewer, etc. — each with its own model fallback chain; most setups start with a single role and never need a second. An in-app hint in `/settings` was considered and deferred: the settings footer is two fixed rows (`FOOTER_ROWS`, pinned by the "footer rows are fixed, never squeezed by body growth" invariant), and adding a third line for a one-time gloss would change layout math for a hint an operator reads once.
...
- **Whether Ollama Cloud caches prefixes at all is stated as unresolved, rather than left to read as a conway shortfall — and "forking is cheap" is now conditioned on a working prefix cache existing** — board item `01M1FH1T944YVJF9ZK3N4M4860`, following up the 2026-09-01 dogfooding session that logged 9.7M input tokens against zero cache reads. A just-landed sibling item (`01M1FSGJM3HAT3493ZRQ2Z4AP0`) already answered part of the original question: Ollama's native `/api/chat` wire format (the path a resolved context window always routes through) carries no cache field of any kind, so the zero was partly an accounting artefact, not proof of a missing cache. What remains genuinely unknown — whether Ollama Cloud's hosted inference performs any server-side prefix caching independent of whether the wire can report it — needs a live measurement this change does not make: no `OLLAMA_API_KEY` was available in the environment that wrote it. `docs/providers.md` states this plainly and gives the exact `curl`-based two-identical-requests recipe for an operator with real credentials to complete it, rather than presenting silence as a finding either way. `docs/agents.md` and `docs/sessions.md` gain the fork-cost honesty declaration this uncertainty implies: a fork child inherits its parent's whole prefix unbounded (`crates/conway-runtime/src/context/path.rs`), so "forking is cheap" holds only under a working prefix cache — on a provider that does not have one, or where conway cannot see one, a fork child resends everything, every turn. `docs/routing.md`'s caching-mechanism claim is softened to match: what conway assumes about a dialect's cache behavior is not the same as a live-confirmed guarantee.

- **The prefix-stability premise the whole prompt-cache economy rests on is now pinned by a test, and `conway.memory`'s own known exception to it is now documented and pinned to an exact number** — board item `01M1FSH18CT765CDD0FKM2F7ZX`. A real session held "each turn's request is the previous one plus new material at the end" across 81 turns before this item existed, with nothing in the suite asserting it: new `crates/conway-runtime/tests/prefix_stability.rs` drives a real `Runtime` through three ordinary turns (each a tool call → tool result → text round trip) and asserts every consecutive `Backend::generate` request is an exact leading prefix of the next (role/content/provenance/tokens_est) with a stable `crate::context::prefix_key`, plus the same property generalized across a fork boundary (a fork child's first request extends its parent's last one as a wire-byte prefix — `Provenance` legitimately differs there, so only `(Role, content)` is compared). Separately, `MemoryInjectHook` splices a `remember`ed memory in BEFORE the request's `ToolRegistry` segment — inside the static prefix, not appended to the volatile tail — so a mid-session `remember` is a real, deliberate exception to the property above; new `crates/conway-plugin-memory/tests/remember_prefix_cost.rs` proves it facade-driven (a scripted `remember` tool call, not a hand-rolled store write) and pins the exact divergence index, not merely "something changed". `docs/plugins/memory.md` gains a "Cache cost" section stating the cost plainly: which turn it lands on, how to see it (the TUI status line's `tokens` cache-percentage parenthetical dips for exactly one turn), and why the insertion point is the front of the request (recall must precede the conversation it informs) rather than the cheaper-for-the-cache tail (which was considered and rejected — it would place a memory after the reasoning it was meant to inform). Ruling: documented, not moved — the insertion point itself is unchanged by this item.
- **What a doc comment is for, with a test a worker can apply line by line** — board item `01M1FSBKJW5NN5ADGYCEF5RN2J`. Comment lines run past half the file in several of the engine's largest modules, and much of that bulk narrates what changed and under which board item rather than what a future editor must not repeat — the reason a reader wanting the rationale for a shape has to dig through history that belongs in the changelog. `CONTRIBUTING.md` §2 gains "What a doc comment is for": KEEP a sentence if deleting it would let a future editor repeat a mistake the author already ruled out; MOVE it to `CHANGELOG.md` (or delete, since the commit already has it) if it only makes sense relative to a prior state — "before this item," "used to," a bare date, a reviewer finding number — quoting `ports/plugin.rs`'s `ToolCtx` doc as the KEEP example and two `crates/conway-cli/src/tui` comments as the MOVE example. It restates the citation rule in this doc comment's terms — a ULID is legitimate provenance but never "this item," and a spec's own `(T3)`-style shorthand is replaced by the fact it abbreviated — and scopes enforcement to the two sweep items this round (`crates/conway-cli/src/tui`, the named engine files) plus ordinary review, review-only rather than a CI gate, consistent with the existing "review-only, not gated" treatment of module docs. No code changed by this item; the sweeps that apply the rule are separate board items.

### Changed

- **The model's own prior turns get a real provenance instead of a fabricated system-note reason** — board item `01M1FSS152J8NQPJAQZV2V2M3K`. Nearly half the segments in a long session are the model's own earlier replies, and `ContextBuilder::build` had no `Provenance` variant for "the model's own prior turn" — `LogRecord::Assistant` carries no `prov` field of its own — so it mapped every one of them to `Provenance::SystemNote { reason: "assistant_turn" }`, a made-up reason indistinguishable in the record from an actual runtime-authored note (`"repeated_step"`, `"result_contract_violation"`). That collision defeated the very discipline `Provenance` exists to provide: a curator driven by this enum (INTENT.md §5a, "mechanical curation driven by provenance") could not tell "the model's own reasoning" from "a system note" and, curating conservatively around system notes, would have deleted the model's entire prior reasoning along with them. `Provenance` gains a sixteenth variant, `Assistant` (no payload, volatile-tier, `is_static() == false`); `own_segment` (`crates/conway-runtime/src/context/builder.rs`) now maps an own `LogRecord::Assistant` to it directly. An assistant turn a fork child INHERITS from its parent is unaffected — `record_role_and_content` discards original provenance for the inherited path regardless of record kind, so it stays `Provenance::Inherited`, exactly like every other own-vs-inherited distinction in this crate. `/context`'s per-segment label and its kind-grouping table (`crates/conway-cli/src/tui/commands.rs`) both render a dedicated `assistant` row, no longer folded into (or fragmenting) the `system note` row; `SegmentKind::from(&Provenance)` (`crates/conway-core/src/segment.rs`) and `provenance_discriminant` (`crates/conway-runtime/src/context/builder.rs`, which also feeds `PromptSegment` id derivation) both gained the matching arm the compiler's own exhaustiveness check forced. `crates/conway-runtime/tests/golden/context_with_steer_and_toolresults.json` is regenerated — its only diff is the one assistant segment's `provenance` field, from `{"type":"system_note","reason":"assistant_turn"}` to `{"type":"assistant"}`; its `content_sha` (computed from content alone) is byte-identical. Wire format: additive only, and this is a read/render-time reclassification, not a rewrite — `LogRecord::Assistant` itself is unchanged, still carries no `prov` field, and no persisted record of any kind is touched. A `SystemNote { reason: "assistant_turn" }` record from a session written before this change still decodes and renders exactly as it always has (the old tag is not retired from the wire, only from what `ContextBuilder::build` newly stamps); only sessions assembled by a binary carrying this change stamp `assistant` going forward.

- **Instruction fragment reachability is now gated per PART, not per whole fragment — so `conway.idiom` can finally say "verify with `bash`"** — board item `01M1FSRJJAB3ZYZXED4SVT2ZSF`. Before this item, `InstructionFragment` had exactly one `tool_ids` list covering its WHOLE text, and `ContextBuilder::build`'s reachability check withheld the entire fragment the instant any one id in it was unreachable — so a fragment that wanted to say "run the tests with `bash` before you claim done" alongside general, always-true orientation text had no way to gate only that one sentence; naming `bash` would have dropped the whole paragraph for a `bash`-less session. `conway.idiom`'s own shipped fragment lived with exactly this trap: it named `conway_fork`/`conway_spawn`/`report` in prose but could never tell the model to actually use one, and deliberately shipped `tool_ids: vec![]` to avoid vanishing from the one session type (an interactive root, no `report`) it exists for. `InstructionFragment` now carries `text: String` (an unconditional body, always rendered) plus `parts: Vec<InstructionPart>` (each `{ text, tool_ids }`, rendered independently — joined onto the body, in declaration order, into the SAME segment — iff every id in ITS OWN `tool_ids` is reachable this turn); the whole-fragment `tool_ids` field is gone. `ContextBuilder::build`'s reachability loop (`filter_reachable_parts`, replacing the old `if unreachable_tool_ids.is_empty()` gate the design-claims predicate below used to pin) now withholds only the unreachable parts, never the body or another reachable part — the fragment is withheld ENTIRELY only when its body is empty and every part gates out, the exact per-part analog of the old all-or-nothing behavior. `InstructionFragmentEntry` (`conway_core::provenance`) gains `withheld_parts: Vec<u16>` (`#[serde(default)]`, so an older session log still decodes); `unreachable_tool_ids` is now the union of ids across every withheld part rather than the whole fragment's own list. `conway-plugin-idiom` gains a small markdown convention (`parse_fragment_markdown`): a paragraph immediately preceded, with no blank line between, by an HTML comment `<!-- tools: id1, id2 -->` is a conditional part; every other paragraph is body — applied identically to the shipped `fragments/idiom.md` (which now ships real, actionable parts: verify with `bash` before claiming done, call `report` to end a turn as a child, fork to a child when the window is filling) and to an operator's own `instructions.md`, so an operator can gate their own sentences the same way. A malformed `<!-- tools: -->` comment is a load error (`FacadeError::Config`), never a silent fallback to body. `conway-plugin-path`/`conway-plugin-discover`'s own single-tool fragments (the only other first-party producers of a non-empty whole-fragment `tool_ids` before this item) migrate to a single part each, body left empty — reachable by construction anyway, since each plugin's own fragment names the one tool that same plugin's `Self::tools` provides. `scripts/board-claims.md`'s reachability predicate is re-pinned from the deleted `if unreachable_tool_ids.is_empty()` literal to the new `filter_reachable_parts` function, and reworded to "a part naming a tool this turn's assembled tool set does not provide never becomes part of a segment" — observed to fail against the OLD predicate (which no longer matches anything, since the gate it named was deleted in this same change) before the re-pin, and to pass after it. `docs/plugins/idiom.md`, `docs/plugins/hooks.md` point 17, and `docs/plugins/authoring.md`'s `instructions()` section are updated to describe body-plus-parts rather than a single whole-fragment `tool_ids` gate.

- **A plugin's own instruction fragment and an operator's own instructions.md text are no longer both durably logged as "a skill"** — board item `01M1FSNBRE5XJ0GQ04RT5HZ1PS`. Every fragment `Plugin::instructions()` contributed used to be stamped `Provenance::Skill { name }` once assembled (`ContextBuilder::build`), the same tag an operator-authored `.conway/skills` body gets — so a shipped plugin paragraph and an operator's own standing instructions (e.g. `conway.idiom`'s `.conway/instructions.md`) were indistinguishable from each other and from an actual skill in the one place a session's durable record lives; the only attribution was a side-channel (`ContextReport::instruction_fragments`), never the segment's own provenance. `InstructionFragment` gains `authored_by: FragmentAuthor` (`Plugin`, the default, or `Operator { path }`); `Provenance` gains two variants, `PluginInstruction { plugin_id, name }` and `Operator { name, path }`, both `is_static`/`Static`-tier exactly like `Skill` was. `ContextBuilder::build` now stamps a fragment by its own `authored_by` — `Skill` itself is untouched and keeps stamping actual skill fragments exactly as before, this item is scoped to instruction fragments only. `conway.idiom` marks its shipped base fragment `FragmentAuthor::Plugin` (unchanged behavior) and its project/global operator files `FragmentAuthor::Operator { path }`; `/context`'s per-segment labels read `plugin:<plugin_id>/<name>` and `operator:<path basename>` respectively, and `conway sessions show`/`export` render the new tags through the same generic `LogRecord`/`Provenance` serialization every other variant already uses, no special-casing needed. `docs/plugins/idiom.md`'s "Seeing it in `/context`" section and its former "provenance limitation" paragraph, `docs/plugins/trust-and-security.md`'s instruction-fragment legibility note, and `provenance.rs`'s own doc comments are updated to describe the new stamps. Wire format: additive only — an older binary reading a newer log fails to decode a `plugin_instruction`/`operator`-tagged record it doesn't know about (the same forward-compat gap every prior `Provenance` addition already had), but every record written before this change, including one using the old `skill` tag, still decodes unchanged; no existing log is rewritten or migrated.

- **`BudgetExceeded`'s `limit` now names which SCOPE it counted, and `AgentResult` carries a second, turn-scoped step counter alongside the session-lifetime one** — board item `01M1FS8R09PGF9HHV95B7A6RMH`. A real session ended `budget_exceeded max_steps=40` with `steps_taken 81`, and nothing said the two numbers counted different things (`max_steps` is turn-scoped for a `keep_alive` agent, `steps_taken` is session-lifetime, always) — reading as though the limit had failed to hold when it had not; the same shape was independently reproduced live, mid-session, by a `keep_alive` root that hit `steps_taken: 76` against `max_steps=40` with zero warning. `ResultStatus::BudgetExceeded { limit }`'s `max_steps`/`max_tool_calls` trips are now suffixed `" (this turn)"` for a `keep_alive` agent or `" (this session)"` otherwise — the exact vocabulary `conway_runtime::runway`'s proactive model-facing warning already used for the same distinction. `AgentResult` gains `steps_this_turn: u32` (`#[serde(default)]`, so a session log written before this field existed still decodes, as `0`) beside the pre-existing `steps_taken`; the TUI's root-session-ended notice, the `jsonl`/`json` renderers, and `conway sessions show` all render both, each labelled with its scope — `docs/scripting.md`'s `json` example and exit-code table, `docs/interactive.md`'s new "When the root session itself ends" section, and `docs/agents.md`'s Budgets section are all updated to match. Reconciling the three `max_steps` defaults (`conway_core::agent::Budget::default()` = `40`; `conway::config::schema::LimitsConfig::default()` and the derived `config::merge::default_document()` = `0`): the disagreement is intentional, not drift, and stays — a root session is watched by a human who can interrupt, so an arbitrary fixed ceiling can only be wrong, while a step ceiling earns its keep on an unwatched subagent; each of the three sites' own doc comment now names the other two and this reasoning, and `docs/agents.md`'s Budgets section states it in prose (and fixes its `settings.json` example, which had drifted to show the stale `40` root default instead of the current `0`).

- **A child's typed facts, artifacts, and structured output now reach the parent's context, not just its one-line summary — and `conway_steer`/`conway_await`'s descriptions say what they actually do** — harness gap review 2026-09-01, finding 3. Before this item, `conway_steer` and `conway_await` were each described in a single sentence ("Send a steering message to a running child agent", "Block for a child agent's terminal result"), and `child_result_text` (`conway-runtime`'s `context/builder.rs`, shared by `conway_await`'s inherited pass-through and a child's non-blocking `ChildResultRecord` notification) rendered only `result.summary` — a child that reported facts, artifacts, or structured output via `report` had all three silently vanish the moment its result crossed into a parent's context. `child_result_text` now replays the whole `AgentResult`: the existing summary line, then a `facts:` section (key, value, source), an `artifacts:` section (kind, id, path), and a `structured:` section (canonical JSON via `conway_core::canon::canonical_json_bytes`) — each omitted when empty, never truncated at render time. `conway_steer`'s description now states delivery happens at the child's next turn boundary and never interrupts an in-flight tool call, that `agent_id` must come from a prior fork/spawn result, and that `conway_cancel` is the alternative when the child should stop outright; `conway_await`'s now names every part of what comes back (summary, facts, artifacts, structured, status) and that an already-finished child returns immediately. The `report` tool's `structured` argument had no size bound at all (only `summary` did, at 2000 characters) — a single call could hand a parent's context an arbitrarily large payload; it now refuses, with a typed error naming the bound, past 65,536 bytes of canonical JSON (matching `fs::read`'s own single-content-item bound), enforced at the producer so the renderer never has to trim. `docs/tools.md`'s subagent section and `report` section, and this entry, are the record of what a parent actually sees.

- **The hook wire types plugin authors build (`HookEvent`, `HookInvocation`, `HookAnswer`, `ContextDelta`) are now `#[non_exhaustive]`, with a constructor replacing struct-literal syntax; the three verdict/policy enums (`HookPermissionVerdict`, `HookOnFailure`, `HookOrigin`) are `#[non_exhaustive]` too, with an unrecognized future variant now failing closed everywhere it's read** — board item `01M1FRV0JDCSC87WRS8QKP1Z5F`. Before this item, `crates/conway-core/src/hook.rs` declared these seven public types with no growth protection at all — adding a field or a variant later would break every third-party plugin's construction/match sites at compile time, silently, with no warning this repo's own precedent (`ports::plugin::ToolCtx`'s deliberate refusal) exists precisely to avoid. Each struct's decision is recorded in its own doc comment: all four took the constructor route (`HookEvent::new`, `HookInvocation::new`, `HookAnswer::new`, `ContextDelta::new`) rather than `ToolCtx`'s refusal, since each is a small, plain-data type where a positional constructor costs about what the literal did. `HookPermissionVerdict::denies()` is now the single implementation of the fail-closed-on-unrecognized-variant judgment this type's own doc requires — a safety-critical classification with exactly one implementation, never re-derived per call site — both call sites that decide whether a verdict blocks a call (`PermissionBroker::pre_tool_use_hook_denial` for `pre_tool_use`, `HookDispatcher::dispatch_deny_only` for `prompt_submitted` and other deny-only events) call it rather than re-deriving the classification; a build-lane review caught that the second site had silently allowed an unrecognized verdict through before this fix, contradicting the type's own "exactly as untrustworthy as an unreachable one" claim. `docs/plugins/hooks.md` point 13 states the evolution contract this establishes.

- **Every config default was written twice — once as a Rust `impl Default`, once as a hand-maintained JSON literal only the second copy actually loaded through — and now it is written once** — harness gap review 2026-09-01, finding 11. `crates/conway/src/config/merge.rs`'s `default_document` used to be its own `serde_json::json!` literal, required to restate every value each of `schema.rs`'s fourteen `impl Default` blocks already named, with nothing enforcing the two agreed: `LimitsConfig::default()` carried `max_tool_calls` while the document's `[limits]` table didn't; `RoutingSection::default()` carried `headroom_fraction` while the document named only `default_headroom_tokens`. Commit `a0f560d` (the `max_steps` `40` -> `0` fix) had to edit both files by hand and said so in its own message — the standing footgun this item retires. New `ConwayConfig::baseline()` (`schema.rs`) is now the single authoritative "nothing configured" value — every section at its own `Default`, `default_role`/`roles` naming the one baked-in floor role — and `default_document` is exactly `serde_json::to_value(ConwayConfig::baseline())`, nothing more. One deliberate special case fell out entirely: `default_document` used to omit `[session].root` by hand to avoid asserting a value there; `Option<PathBuf>`'s `Deserialize` reads an explicit `"root": null` (what `SessionConfig::default()` now serializes to) identically to the key being absent, so the omission was unnecessary and is gone along with the comment that justified it. Changing a section's default is now one edit, in one file, with a compile error on omission (`ConwayConfig::baseline()`'s struct literal names every field) — not two edits in two files with no check that they still agree. New `crates/conway/tests/config_defaults_single_source.rs` is the divergence guard: it loads a settings file naming nothing through the real production loader (`conway::config::load`) and asserts the result equals `ConwayConfig::baseline()` field by field, then separately round-trips each top-level section of the built-in document (read back through the existing public `conway::config::merged_document` seam, never the private `default_document` directly) into that section's own type and asserts it equals that type's own `Default::default()`.

- **The seven knobs every way of starting an agent shares — `agent_def`/`role`/`model`/`tools`/`budget`/`result_contract`/`keep_alive` — are now one struct composed into each spec, not seven repeated fields; adding a per-agent knob is now a one-field, one-spread change instead of a scatter across six structs and their conversions** — board item `01M1FSPP9D80FNXH9QFZ9927DH`. New `conway_core::agent::AgentKnobs` holds the seven; `SubagentSpec` embeds it via `#[serde(flatten)]` (wire form byte-for-byte unchanged — pinned by a committed fixture, `crates/conway-core/tests/subagent_spec_wire_identity.rs`, written and green against the OLD shape before this item's own refactor commit, still green after it), `conway_runtime::runtime::RootSpec`/`ResumeSpec` and `conway`'s `ForkSpec`/`SpawnSpec` embed it as a plain `knobs: AgentKnobs` field. Canonical field name for the model-pin knob is `model`, not `pin` — three of the four non-`SubagentSpec` specs already spelled it that way; `SubagentSpec::pin` was the one outlier, and it alone keeps its wire key via `#[serde(rename = "pin")]` on `AgentKnobs::model`, rather than renaming everywhere else. `SessionSpec` deliberately does NOT embed `AgentKnobs` — its `budget` is `Option<Budget>`, deferred to `ConwayConfig` at `Conway::new_session` time, and `AgentKnobs::budget` is a required, already-resolved `Budget`; embedding directly would have silently changed `SessionSpec::default()`'s effective budget from "read `config.limits` (frequently unbounded)" to "`Budget::default()`'s 40 steps." `Conway::new_session` instead builds one `AgentKnobs` value from `SessionSpec`'s resolved fields and spreads it into `RootSpec::knobs` in a single expression. `ResumeSpec` was considered for folding into `RootSpec` (left to this item's own worker to verify) and kept separate: beyond `session` being required vs. optional, `RootSpec` alone carries `prompt` (resuming never appends an initial turn), `root` (a resumed session's root always comes from its persisted `SessionMeta`, never a fresh override), `system_prompt_override`, and `labels`, and `cwd` itself differs in kind (`RootSpec`: required `PathBuf`; `ResumeSpec`: `Option<PathBuf>` override) — folding would need most of `RootSpec`'s own fields to become optional on a merged type, a bigger and riskier shape change than composing a shared knobs struct. **What an external embedder needs to know:** every existing builder method (`ForkSpec`/`SpawnSpec`'s `.agent_def()`/`.role()`/`.model()`/`.tools()`/`.budget()`/`.result_contract()`/`.keep_alive()`) keeps its exact pre-existing signature and behavior — a call-site chain like `ForkSpec::new("...").role(r).budget(b)` compiles and behaves unchanged, no migration needed. The one break: code that sets these seven fields directly on a `ForkSpec`/`SpawnSpec` struct literal (bypassing the builder methods) needs to nest them under `.knobs` instead (`spec.role = r` becomes `spec.knobs.role = r`) — `docs/embedding.md`'s own `Conway::fork_from` example hit exactly this and is updated in this same change (`spec.result_contract = Some(contract)` replaced by the equivalent `.result_contract(contract)` builder call, which is the recommended form either way). `ForkSpec`/`SpawnSpec::agent_def`'s underlying type also changed, `Option<String>` to `Option<AgentDefRef>` (matching `AgentKnobs`) — invisible through the `.agent_def(impl Into<String>)` builder method, which still takes a plain string and constructs the ref internally. `AgentSpec` (`conway_runtime::agent_loop`) is untouched, per this item's own scope — it stays built from resolved locals at its two construction sites in `root.rs`/`subagent.rs`, each now a single spread of the resolved values plus its own per-primitive fields.
...
- **A plugin's instruction fragment can now render ahead of an agent def's own system prompt, and can be scoped to root-only or children-only — `conway.idiom`'s base fragment now renders genuinely first, not merely first among what follows `[0]`** — harness gap review 2026-09-01, finding 2 (process record `01M1FQ36PCW2J19AP219GKZH3R`). Before this item, every `Plugin::instructions()` fragment landed at a single fixed slot, AFTER `[0] SystemPrompt` (an agent def's own prompt, or a one-shot `--system-prompt`/`--append-system-prompt` override) and BEFORE an operator's own directory-authored skills — `conway.idiom`'s own module doc argued at length that this was "the right place" and that landing ahead of `[0]` "does not change `ContextBuilder::build`'s assembly order... a follow-up, not something this plugin does by reordering itself." `conway_core::ports::plugin::InstructionFragment` gains `position: FragmentPosition` (`BeforeSystemPrompt`/`AfterSystemPrompt`, default `AfterSystemPrompt` — today's exact behavior), `order: i16` (lower renders first within a position; default `0`; a tie keeps install order), `scope: FragmentScope` (`All`/`RootOnly`/`ChildrenOnly`, default `All` — every fragment still reaches every agent by default), and `agent_def: Option<String>` (render only when `[0]`'s own agent-def name matches). `InstructionFragment::new(name, text)` returns every field at its default, plus `with_tool_ids`/`with_position`/`with_order`/`with_scope`/`with_agent_def` builders, mirroring `PromptSegment`'s own `with_cache_hint`/`with_tokens_est` precedent; the struct stays exhaustive (no `#[non_exhaustive]`) so every existing construction site still compiles as a plain literal once the new fields are named. `ContextInput` gains `agent_kind: AgentKind` (`Root`/`Child`), sourced by `AgentLoop` from the STRUCTURAL fact `Self::parent.is_none()` — never inferred from a tool set or a role. `ContextBuilder::build` stable-sorts `input.instructions` by `(position, order, install_index)` (deterministic — a plain numeric sort, no hashing, so the assembled prefix stays byte-identical turn over turn, the property `crates/conway-runtime/tests/prefix_stability.rs` pins end to end); `BeforeSystemPrompt` fragments render ahead of `[0]`, `AfterSystemPrompt` fragments where `[1]` always has; a fragment whose `scope`/`agent_def` excludes this turn's agent is dropped before rendering and recorded with a new `skipped_by_scope: bool` on `InstructionFragmentEntry` (kept separate from `unreachable_tool_ids` — the two answer different operator questions). `conway.idiom`'s shipped base fragment now declares `BeforeSystemPrompt`/`order: -100` — it renders ahead of even a curated `AgentDef`'s own prompt; its operator project/global fragments stay at the `AfterSystemPrompt` default. **No role selector was built** — no first-party consumer needs one, and threading a role into `ContextInput` for a selector nothing uses would be unbuilt theory; disclosed in `docs/plugins/hooks.md` point 17. `docs/plugins/idiom.md`, `docs/plugins/hooks.md` point 17, `docs/plugins/trust-and-security.md`'s "Instruction fragments" section, and `docs/scripting.md`'s `--system-prompt` section are corrected to describe the new, position-dependent ordering rather than the old fixed one.
